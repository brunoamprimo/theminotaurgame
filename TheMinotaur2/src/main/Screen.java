...
public class Screen {
    ...
    public void setMap(int[][] nmap) { // Updates the map for minotaurs
    	map = nmap;
    }
    
    public void setup(int[][] m, int mapW, int mapH, ArrayList<Texture> tex, int w, int h, int mapS) {
    	map = m;
		mapWidth = mapW;
		mapHeight = mapH;
		textures = tex;
    ...
    public int[] update(Camera camera, int[] pixels) {
		for(int n=0; n<pixels.length/2; n++) {
			if(mapSelected == 2) {
				...
				if(pixels[n] != Color.BLUE.getRGB()) pixels[n] = Color.BLUE.getRGB();
			}else {
